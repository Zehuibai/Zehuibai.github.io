...
### Test: From *all2* to *high2* data set
...
```{r}
#| echo: false
high2_expand.cc <- high2_expand %>% dplyr::filter(dropout_grp=="Completer")

fit_cc <- mmrm::mmrm(
  formula = change ~ basval*avisit + trt*avisit + us(avisit | subject),
  data = high2_expand.cc,
  control = mmrm_control(method = "Kenward-Roger")
)
...
#### Summary of results *high2_expand* data set
